use crate::{
    board::Board,
    types::{Bitboard, Color, File, Piece, PieceType, Square, ZOBRIST},
};
...
pub fn all_attacks(board: &Board, color: Color) -> Bitboard {
    ...
    let mut threats = pawn_attacks_setwise(board.pieces(PieceType::Pawn) & our, color);
    for square in our & board.pieces(PieceType::Knight) {
        threats |= knight_attacks(square)
    }
    threats |= king_attacks(board.king_square(color));

    #[cfg(target_feature = "avx512f")]
    {
        use crate::lookup::slider_attacks_setwise;
        threats |= slider_attacks_setwise(
            our & board.pieces(PieceType::Bishop),
            our & board.pieces(PieceType::Rook),
            our & board.pieces(PieceType::Queen),
            board.occupancies(),
        );
    }
    ...
    {
        for square in our & (board.pieces(PieceType::Bishop) | board.pieces(PieceType::Queen)) {
            threats |= bishop_attacks(square, board.occupancies());
        }

        for square in our & (board.pieces(PieceType::Rook) | board.pieces(PieceType::Queen)) {
            threats |= rook_attacks(square, board.occupancies());
        }
    }
