...
fn search<NODE: NodeType>(
    ...
    if best_move.is_some() {
        let scale_len = |len: usize| (len + len * len / 3).min(32) as i32;
        let noisy_bonus = (106 * depth).min(808) - 54 - 80 * cut_node as i32;
        let noisy_malus = (164 * depth).min(1329) - 52 - 15 * scale_len(noisy_moves.len());

        let quiet_bonus = (172 * depth).min(1459) - 78 - 54 * cut_node as i32;
        let quiet_malus = (144 * depth).min(1064) - 45 - 20 * scale_len(quiet_moves.len());

        let cont_bonus = (108 * depth).min(977) - 67 - 52 * cut_node as i32;
        let cont_malus = (352 * depth).min(868) - 47 - 10 * scale_len(quiet_moves.len());
